...
import com.hl3hl3.arcoremeasure.BuildConfig; // добавил
            ...
            try {
                ...
                if(anchors.size() < 1){
                    ...
                    for(int i = 1; i < anchors.size(); i++){
                        ...
                        float distanceCm = ((int)(getDistance(point0, point1) * 1000))/10.0f; // тут подсчет как я понимаю
                        total += distanceCm;
                        areacm = 2 * total; // добавил
                    ...
                    // show result
                    String result = sb.toString().replaceFirst("[+]", "") + " = " + (((int)(total * 10f))/10f) + "cm" + " / "+ (((int)(areacm * 10f))/10f) + ""  ;
                    showResult(result);
